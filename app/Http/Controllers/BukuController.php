...
class BukuController extends Controller
{
    ...
    public function update(Request $request, $id){
        ...
        // Process categories array and store it in the database
        $categoriesArray = [];
        foreach ($request->categories as $category) {
            $categoriesArray[] = ['nama_kategori' => $category];
        }
        $buku->categories()->delete(); // Delete existing categories
        $buku->categories()->createMany($categoriesArray);
    ...
    public function store(Request $request) {
        ...
        if ($request->input('categories')) {
            ...
            foreach ($request->input('categories') as $categoryName) {
                $categoriesArray[] = ['nama_kategori' => $categoryName];
            }
            $buku->categories()->createMany($categoriesArray);
        }
    ...
    public function showPopulerBuku()
    {
    // Get 10 books with the highest average rating
    $topBooks = Buku::withAvg('ratings', 'rating')
                    ->orderByDesc('ratings_avg_rating')
                    ->limit(10)
                    ->get();

    return view('buku.populer', compact('topBooks'));
    }
